...
abstract class wds_form extends moodleform implements generic_states {
    public $current;
    public $next;
    public $prev;
    ...
    public static function create($prefix, $courses, $state = null, $extra= null) {
        ...
        if ($state == self::LOADING) {
            return new wds_loading_form($extra);
        }
    ...
    public function display_course($course, $sem) {
        ...
        return "$semestername $course->course_subject_abbreviation $course->course_number";
    ...
    public function display_semester($sem) {
        ...
        return "$sem->period_year $sem->period_type$session";
    }
...
class wds_loading_form implements generic_states {
    ...
    public function display() {
        ...
        // $PAGE->requires->js('/blocks/wdsprefs/js/loading.js');
        $PAGE->requires->js_call_amd('block_wdsprefs/loading', 'init');
        $s = wds::gen_str('block_wdsprefs');

        echo $OUTPUT->box_start('generalbox wds_loading');
