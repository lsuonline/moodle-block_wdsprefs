...
abstract class wds_dalo extends wds_base implements meta_information {

    /** Public api to interact with wds_dalo's */
    public static function get_meta($parentid) {
        global $DB;

        $params = array(self::call('get_name'). '_listing_id' => $parentid);
    ...
    */
    ...
    public static function get_all($params = array(), $meta = false, $sort = '', $fields = '*', $offset = 0, $limit = 0) {
        ...
        // DALO: FIX THIS, not linking meta table.
    ...
    /**
     * @param array | wds_dalo_filter_builder $params
     *
     */
    ...
    public static function metatablename($alias = '') {
        $name = sprintf('enrol_%s_meta', get_called_class());
    ...
    public static function delete_all($params = array()) {
        ...
        if (self::call('params_contains_meta', $params)) {
            ...
            return self::delete_all_internal(wds::where()->id->in($ids), $handler);
        } else {
    ...
    public function save($params = '') {
        ...
        // TODO: DALO: we don't need to save section meta, but not sure about other things.
        // $this->save_meta($meta);
    ...
    public function save_meta($meta) {
        ...
        foreach ($meta as $name => $value) {
            ...
            $m->{$parentref. $listing_id} = $this->id;
            // $m->name = $name;
            // $m->value = $value;
